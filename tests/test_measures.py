...
def test_relative_bias(gosset):
    ...
    np.testing.assert_array_almost_equal(test, np.array([[0.02366494, 0.16392256, 0.01920133]]))
...
def test_ratio(gosset):
    ...
    np.testing.assert_array_almost_equal(test, np.array([[1.023665, 1.1639225, 1.0192013]]))
...
def test_annual_cycle_correlation(gosset):
    sim = xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc")).sel(time=slice("1950", "1953")).tasmax
    ref = xr.open_dataset(gosset.fetch("sdba/nrcan_1950-2013.nc")).sel(time=slice("1950", "1953")).tasmax
    test = measures.annual_cycle_correlation(sim, ref, window=31).sel(location="Vancouver").values
    np.testing.assert_array_almost_equal(test, [0.94580488], 4)


@pytest.mark.slow
def test_scorr(gosset):
    ref = xr.open_dataset(gosset.fetch("NRCANdaily/nrcan_canada_daily_tasmin_1990.nc")).tasmin
...
def test_taylordiagram(gosset):
    sim = xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc")).sel(time=slice("1950", "1953"), location="Amos").tasmax
    ref = xr.open_dataset(gosset.fetch("sdba/nrcan_1950-2013.nc")).sel(time=slice("1950", "1953"), location="Amos").tasmax
