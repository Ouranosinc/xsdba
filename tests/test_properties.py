...
class TestProperties:
    def test_mean(self, gosset, use_dask):
        sim = (
        ...
        assert out_year.attrs["property"] == "mean"
    ...
    def test_std(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1980"), location="Vancouver")
            .pr
    ...
    def test_skewness(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1980"), location="Vancouver")
            .pr
    ...
    def test_quantile(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1980"), location="Vancouver")
            .pr
    ...
    def test_bivariate_spell_length_distribution(self, window, expected_amount, expected_quantile, gosset, use_dask):
        ds = xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={}).sel(
            time=slice("1950", "1952"), location="Vancouver"
        )
        if not use_dask:
    ...
    def test_acf(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
    ...
    def test_annual_cycle(self, gosset, use_dask):
        simt = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1952"), location="Vancouver")
            .tasmax
    ...
    def test_annual_range(self, gosset, use_dask):
        simt = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1952"), location="Vancouver")
            .tasmax
    ...
    def test_corr_btw_var(self, gosset, use_dask):
        simt = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1952"), location="Vancouver")
            .tasmax
    ...
    def test_transition(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1952"), location="Vancouver")
            .pr
    ...
    def test_trend(self, gosset, use_dask):
        simt = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1950", "1952"), location="Vancouver")
            .tasmax
    ...
    def test_return_value(self, gosset, use_dask):
        simt = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
    ...
    @pytest.mark.slow
    def test_spatial_correlogram(self, gosset, use_dask):
        ...
        sim = xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={}).sel(time=slice("1981", "2010")).tasmax
        if not use_dask:
            sim = sim.load()
        ...
        sim = xr.open_dataset(gosset.fetch("NRCANdaily/nrcan_canada_daily_tasmax_1990.nc"), chunks={}, engine="h5netcdf").tasmax
        if use_dask:
            sim = sim.load()
        out = properties.spatial_correlogram(sim.isel(lon=slice(0, 50)), dims=["lon", "lat"], bins=20)
        np.testing.assert_allclose(out[:5], [0.95099902, 0.83028772, 0.66874473, 0.48893958, 0.30915054], rtol=5e-07)
        np.testing.assert_allclose(
            out.distance[:5],
    ...
    def test_spectral_variance(self, gosset, delta, expected, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("NRCANdaily/nrcan_canada_daily_tasmax_1990.nc"), engine="h5netcdf", chunks={})
            .tasmax.isel(time=0)
    ...
    def test_normalized_radial_wavenumber_conversion(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("NRCANdaily/nrcan_canada_daily_tasmax_1990.nc"), engine="h5netcdf", chunks={})
            .tasmax.isel(time=0)
            .sel(lat=slice(50, 49.5), lon=slice(-80, -79.5))
    ...
    def test_get_measure(self, gosset, use_dask):
        sim = (
            xr.open_dataset(gosset.fetch("sdba/CanESM2_1950-2100.nc"), engine="h5netcdf", chunks={})
        ...
        ref = (
            xr.open_dataset(gosset.fetch("sdba/ahccd_1950-2013.nc"), engine="h5netcdf", chunks={})
            .sel(time=slice("1981", "2010"), location="Vancouver")
            .pr
        )
        if not use_dask:
